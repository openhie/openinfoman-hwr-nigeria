...
return
  if (count($provs1) = 1 and exists($id)) 
	...
	if (exists($careServicesRequest/requestParams/otherID/@issueDate) )
	  then 
	    if (exists($id/@issueDate))
	      then replace value of node $id/@issueDate with $careServicesRequest/requestParams/otherID/@issueDate
	      else insert node $careServicesRequest/requestParams/otherID/@issueDate into $id		
	  else (),
	if (exists($careServicesRequest/requestParams/otherID/@expirationDate) )
	  then 
	    if (exists($id/@expirationDate))
	      then replace value of node $id/@expirationDate with $careServicesRequest/requestParams/otherID/@expirationDate
	      else insert node $careServicesRequest/requestParams/otherID/@expirationDate into $id		
	  else (),
